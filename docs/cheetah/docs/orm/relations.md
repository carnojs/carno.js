...
Cheetah.js ORM supports standard relationships using `ManyToOne` and `OneToMany` decorators.
...
```ts
// post.entity.ts
import { Entity, Property, BaseEntity, ManyToOne, Ref } from '@cheetah.js/orm';
...
```ts
// user.entity.ts
import { Entity, Property, BaseEntity, OneToMany, Ref } from '@cheetah.js/orm';
