...
Cheetah.js has a powerful Dependency Injection (DI) system that manages the instantiation and lifecycle of your classes.
...
To make your services and controllers available to the container, you must register them. While you can register everything in the root `Cheetah` instance, it is highly recommended to use **Plugins** to organize your code into modules.
...
```ts
import { Service, ProviderScope } from '@cheetah.js/core';
...
new Cheetah({ providers: [configProvider] });
```
